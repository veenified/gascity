...
| SESSION-START-010 | Terminal named identity release | Terminal close, including reconciler drain close, closes only the observed session instance while releasing its alias, runtime name, explicit-name marker, pending-create claim, and canonical identity. Identifier lock artifacts are removed safely after use, and startup cleanup repairs stale closed-session claims and unlocked legacy lock files. | `internal/session/store_test.go`; `internal/session/names_test.go`; `internal/session/name_claim_sweep_test.go`; `cmd/gc/session_reconciler_test.go` |
